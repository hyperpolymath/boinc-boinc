...
This document explains how all operations in the Oblibeny BOINC Platform
are reversible, in accordance with the MPL-2.0 v0.8 and RSR (Rhodium
Standard Repository) principles.

=== Core Principle

*Every action can be undone.* This eliminates fear of experimentation
and enables learning from mistakes without permanent consequences.
...
Require pull request reviews (2 approvals minimum) - Require status
checks to pass - Require signed commits - No force-push allowed - No
deletion allowed

==== Tag Protection

Release tags are immutable: - Once created, never deleted - Signed with
PGP keys - Verifiable with `+git tag -v <tag-name>+`
...
Critical operations use event sourcing: - Work unit creation - Result
validation - Credit assignment - Governance votes
...
*Recovery*: Invalid configs rejected at deploy time, no manual rollback
needed.
...
[arabic]
. Request deletion → Account marked `+deleted_at+`
. 30-day grace period → Account hidden but data preserved
. After 30 days → Hard delete (data exported first)
...
*Timeframe*: No time limit on reinstatement (pending current team
consensus).
...
[arabic]
. Rotate immediately (automated)
. Audit all actions with compromised credentials
...
[arabic]
. Revert commit
. Force-push to protected branch (requires 2 maintainers)
. Notify community
. Security audit
...
[arabic]
. Revoke PGP signature
. Delete release artifacts
. Create new patched release
. Issue CVE
. Notify all users
...
*Includes*: - User profile - Contributions (git history) - Work units
and results - Credit and statistics - Preferences and settings
...
Once a commit is signed, the signature cannot be "`unsigned`" (but
...
Once information is publicly disclosed (e.g., security vulnerability),
...
After retention period (90 days for soft-deleted data), hard deletion is
permanent.
...
Changes to external dependencies (npm, cargo, etc.) cannot be controlled
by us.
...
____
"`The palimpsest is a manuscript page that has been scraped off and used
again. The underlying text can often still be read through the newer
text.`"
____

Like a palimpsest, our system preserves layers of history. Nothing is
truly erased, only overlaid. This enables:

[arabic]
. *Learning*: Mistakes are learning opportunities, not catastrophes
. *Trust*: Users trust a system that won’t trap them
. *Experimentation*: Innovation requires safety to fail
. *Accountability*: History shows who did what and why
. *Resilience*: Systems that can rewind are more robust
...
* LICENSE.txt - MPL-2.0 v0.8
* GOVERNANCE.adoc - Governance reversibility
* SECURITY.md - Security incident reversibility
* CONTRIBUTING.adoc - Contribution process

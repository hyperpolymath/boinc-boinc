...
All notable changes to the Oblibeny BOINC Platform will be documented in
this file.
...
=== https://gitlab.com/oblibeny/boinc/compare/v0.6.0...HEAD[Unreleased]
...
=== https://gitlab.com/oblibeny/boinc/releases/v0.6.0[0.6.0] - 2024-11-22
...
We use Semantic Versioning (SemVer): - *MAJOR*: Incompatible API
changes, grammar breaking changes - *MINOR*: New features,
backward-compatible additions - *PATCH*: Bug fixes, documentation
updates
...
. Update version in all Cargo.toml, mix.exs, etc.
. Run full test suite (when implemented)
. Tag release: `+git tag v0.6.0+`
. Push tag: `+git push --tags+`
. CI/CD builds and publishes artifacts
...
*RSR Compliance*: Bronze (Documentation category) *Maintained by*: See
MAINTAINERS.md
